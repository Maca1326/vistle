...
SplitDimensions::SplitDimensions(const std::string &name, int moduleID, mpi::communicator comm)
   ...
   p_reuse = addIntParameter("reuse_coordinates", "do not renumber vertices and reuse coordinate and data arrays", false, Parameter::Boolean);
}
...
bool SplitDimensions::compute(std::shared_ptr<PortTask> task) const {
   ...
   if (ugrid) {
       for (int d=1; d<=3; ++d) {
           if (out[d]) {
               ...
               std::cerr << "remapped from " << ugrid->getNumCorners() << " to " << vm[d].size() << " vertices" << std::endl;
